...
def main():
    """
    Sends sample charging data for port_3 to the Firebase Realtime Database.
    """
    # --- Configuration ---
    ...
    session_id = f"session_py_{start_timestamp}"
    ...
    # 3. Send log entries to simulate charging
    print(f"Simulating live charging data for '{port_id}'. Press Ctrl+C to stop.")
    try:
        while True:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Sent -> V: {voltage}V | A: {current}A")
            time.sleep(5)
        ...
        print(f"\nCharging simulation complete. Session '{session_id}' marked as 'completed'.")
